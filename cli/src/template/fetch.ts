...
/**
 * Downloads `templates/<projectType>` from a GitHub ref as a tarball and extracts
 * just that subtree. No git checkout, no extra dependency beyond `tar`.
 */

export const DEFAULT_TEMPLATE_REPOSITORY = 'AmeriLux-Dev/create-netsuite-project';
...
/** Keep only `<archive-root>/templates/<projectType>/**`; the root folder is `<repo>-<ref>` and is stripped along with `templates/<type>`. */
export function isTemplateArchiveEntry(entryPath: string, projectType: string): boolean {
    ...
    return segments.length >= 4 && segments[1] === 'templates' && segments[2] === projectType;
}
...
export async function downloadTemplate(options: TemplateFetchOptions): Promise<void> {
    ...
    if (extracted.length === 0) {
        throw new TemplateFetchError(
            `The archive at ${options.repository}@${options.ref} has no templates/${options.projectType} folder.`,
            url,
        );
    }
}
