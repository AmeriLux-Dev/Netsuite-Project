...
import { buildTemplateArchiveUrl, downloadTemplate, isTemplateArchiveEntry, TemplateFetchError } from '../src/template/fetch.js';
...
describe('isTemplateArchiveEntry', () => {
    it('keeps only the requested template subtree', () => {
        expect(isTemplateArchiveEntry('repo-main/templates/react-app/package.json', 'react-app')).toBe(true);
        expect(isTemplateArchiveEntry('repo-main/templates/react-app/api/src/x.ts', 'react-app')).toBe(true);
        expect(isTemplateArchiveEntry('repo-main/templates/scripts-only/package.json', 'react-app')).toBe(false);
        expect(isTemplateArchiveEntry('repo-main/cli/package.json', 'react-app')).toBe(false);
        expect(isTemplateArchiveEntry('repo-main/templates/react-app', 'react-app')).toBe(false);
    });
});
...
describe('downloadTemplate', () => {
    ...
    async function buildArchive(): Promise<Buffer> {
        ...
        await fs.mkdir(path.join(archiveRoot, 'templates', 'react-app', 'api'), { recursive: true });
        await fs.mkdir(path.join(archiveRoot, 'templates', 'other'), { recursive: true });
        await fs.mkdir(path.join(archiveRoot, 'cli'), { recursive: true });
        await fs.writeFile(path.join(archiveRoot, 'templates', 'react-app', 'package.json'), '{}');
        await fs.writeFile(path.join(archiveRoot, 'templates', 'react-app', 'api', 'a.ts'), 'export {};');
        await fs.writeFile(path.join(archiveRoot, 'templates', 'other', 'package.json'), '{}');
        await fs.writeFile(path.join(archiveRoot, 'cli', 'secret.ts'), 'nope');
    ...
    it('extracts only templates/<type>, stripping the leading folders', async () => {
